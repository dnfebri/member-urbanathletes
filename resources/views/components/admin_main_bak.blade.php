...
<body x-data class="h-screen mx-auto antialiased flex justify-between">
  ...
  <button @click="$store.sidebar.navOpen = !$store.sidebar.navOpen"
    class="sm:hidden absolute top-5 left-5 focus:outline-none">
    <!-- Menu Icons -->
  ...
  <div class="h-screen bg-gray-900 transition-all duration-300 space-y-2 fixed sm:relative"
    ...
    <button @click="$store.sidebar.navOpen = !$store.sidebar.navOpen"
      ...
      <!-- Close Menu -->
      <svg x-cloak xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white"
        x-bind:class="$store.sidebar.navOpen ? '':'hidden'" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
      </svg>
    </button>
    ...
    <div class="px-4 space-y-2">
      ...
      <div x-data="dropdown" class="relative">
        <!-- Dropdown head -->
        <div @click="toggle('audience')" x-data="tooltip" x-on:mouseover="show = true" x-on:mouseleave="show = false"
      ...
      <div x-data="dropdown" class="relative">
        <!-- Dropdown head -->
        <div @click="toggle('promote')" x-data="tooltip" x-on:mouseover="show = true" x-on:mouseleave="show = false"
          class="flex justify-between text-gray-400 hover:text-gray-200 hover:bg-gray-800 items-center space-x-2 rounded-md p-2 cursor-pointer"
